...
axiosInstance.interceptors.request.use(config => {
    ...
    // 如果不是http/htts开头，加入前缀
    if (!config.url.startsWith('http://') && !config.url.startsWith('https://')) {
        if (window.PROJECT_CONFIG.BK_SUB_PATH) {
            config.url = `${AJAX_URL_PREFIX}/${window.PROJECT_CONFIG.BK_SUB_PATH}${config.url}`
        } else {
            config.url = `${AJAX_URL_PREFIX}${config.url}`
        }
    }
